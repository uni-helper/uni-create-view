...
export default async function createUniAppView(options: EcreateUniAppView) {
  const {
    create_path, viewName, pageName, component,
    typescript, styleType, subcontract, directory,
    compositionApi
  } = options;
  ...
  try { directory && fs.mkdirSync(path.resolve(create_path, viewName)); }
  ...
  fs.writeFile(
    ...
    createV2ViewTemplate({ viewName, typescript, styleType, component, compositionApi }),
    { flag: "w" }, () => { }
  );
